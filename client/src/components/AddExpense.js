...
const AddExpense = ( {participants, tripId, baseCurrency} ) => {
    ...
    const handleCurrencyChange = (key, value) => {
        ...
        if (currencyData.currency  && key === "currencyDate") {
                fetch(`https://travel-tracker-server.vercel.app/getHistoricalRate/${currencyData.currency}/${baseCurrency}/${value}`)
                .then(res => res.json())
                .then((data) => {
        ...
        if (currencyData.currencyDate && key === "currency") {
                fetch(`https://travel-tracker-server.vercel.app/${value}/${baseCurrency}/${currencyData.currencyDate}`)
                .then(res => res.json())
                .then((data) => {
                    setCurrencyData({...currencyData, [key]: value, rate: data.data.result})
                    if (currencyData.amount) {
    ...
    const addExpense = async () => {
        ...
        try {
            ...
            const response = await fetch(`https://travel-tracker-server.vercel.app/addExpense/${tripId}`, {
                method: "POST",
                headers : {
